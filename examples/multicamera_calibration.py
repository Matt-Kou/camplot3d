...
scene = Scene()
scene.render(plot=True, title="initial scene")
num_cam = 2
num_points = 4
num_epoch = 10000
render_step = 1200
...
# plot current scene
# white lines are the lines connecting the camera and the anchor corners
# blue lines are the lines connecting the camera and the calibration points
scene.render(plot=True, trace=True, title="scene with camera and calibration points")

with open("data/scene", 'wb') as f:
    pickle.dump(scene, f)
...
for e in range(num_epoch):
    if e % render_step == 0 or e == num_epoch - 1:
        with torch.no_grad():
            pl = scene.render(plot=False, trace=True)
            scene_train.render(pl=pl, plot=True, trace=True, error=False, title=f"train epoch {e}",
    ...
    for cam_idx, camera in enumerate(scene_train.cameras()):
        print(cam_idx, camera.cam_position.tolist(), camera.fov_h.item())
        if cam_idx == 0:
...
scene_train.detach()
scene_train.render(plot=True, title="Final training scene")
scene.render(plot=True, title="True scene")

with open("data\\scene_train", 'wb') as f:
    pickle.dump(scene_train, f)
